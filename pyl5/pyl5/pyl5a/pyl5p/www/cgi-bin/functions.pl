...
{
	#los permisos es para evitar tanto problema con escritura de archivos....
	`chmod 777 $WEB_ROOT_PATH/output/images/$_[2]/`;
	...
	else
	{
		...
		`rm $WEB_ROOT_PATH/output/images/$_[2]/outputcard.mpg`;
		
		open FILE, ">$WEB_ROOT_PATH/output/images/$_[2]/mail.properties" or die $!;
		print FILE "[section_name]\n";
		print FILE "from=$_[3]\n";
		...
		##sera este el que se utiliza realmente para SoundFilme???
		`$ROOT_PATH/SoundFilm/ffmpeg -i $WEB_ROOT_PATH/output/images/$_[2]/output.mpg -vf "drawtext=fontfile=/usr/share/fonts/truetype/freefont/FreeSans.ttf: text='$_[7]': r=50: \ x=(w-tw)/2: y=h-(2*lh): fontcolor=black: box=0.5: boxcolor=#50FFFFFF" -y $WEB_ROOT_PATH/output/images/$_[2]/outputcard.mpg`;

		
		#Se que no es la mejor forma de hacerlo pero por ahora toca así.
...
sub ExecuteMailSender
{
	`chmod -R 777 $WEB_ROOT_PATH/output/images/$_[0]/`;
	#Se que no es la mejor forma de hacerlo pero es mejor que reiniciar.
	`/usr/lib/jvm/java-7-openjdk/bin/java -jar $ROOT_PATH/EMailGenerator/PYLMail.jar "$WEB_ROOT_PATH/output/images/$_[0]/mail.properties" "$ROOT_PATH/EMailGenerator/mailsender.properties"`;
}
...
print("
	...
	#`./writeMini.sh '$radioActors' '/var/www/pyl5/www/cgi-bin/soundsActors.txt'`;
...
sub GetDefaultActorNumber
{
	my ($folder) = @_;
	my $defaultText = "";
	my $script_filename = $WEB_ROOT_PATH.'/output/images/'.$folder.'/script.xml';
	
	my $parser = new XML::DOM::Parser;
	my $doc = $parser->parsefile ($script_filename);

	#my $compositions = $doc->getFirstChild;
	#my $composition = $compositions->getFirstChild;
	#my $actors = $composition->getFirstChild;
	#my $actor = $actors->getElementsByTagName("actor");
	
	my $actor = $doc->getElementsByTagName("actor");
	my $n = $actor->getLength;
...
sub ExecuteSoundFilm
{
	...
	print `./SoundFilm.sh '$IMAGE_PROCESSING_BASE/Execution/vocabulary.xml' '$WEB_ROOT_PATH/output/images/$_[0]/script.xml' '$WEB_ROOT_PATH/output/images/$_[0]/score.xml' '$IMAGE_PROCESSING_BASE' '$WEB_ROOT_PATH/output/images/$_[0]/SoundFilm.log' '$ROOT_PATH/SoundFilm/' '$ROOT_PATH/SoundFilm/bin'`;
}
